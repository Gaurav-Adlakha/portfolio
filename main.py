...
clean_styles = Style("""
    body { font-family: 'IBM Plex Sans', sans-serif; background: #fff; color: #374151; }
    code, pre { font-family: 'IBM Plex Mono', monospace; }
    section, main, div { background: #fff !important; }
""")
...
def create_improved_hero():
    ...
    return Main(
        intro,
        # recent_posts_section(),  # Temporarily disabled
        cls="w-full max-w-2xl mx-auto px-6 py-8 space-y-8"
    )

def recent_posts_section():
    """Show 3 most recent blog posts on home page - like reference website"""
    metadata_list = get_all_metadata()
    if not metadata_list: return Div()
    
    # Sort by date and take first 3
    sorted_posts = sorted(metadata_list, key=lambda x: x.get('date', ''), reverse=True)[:3]
    ...
    def post_item(meta):
        ...
        return Div(
            A(title, href=f"/posts/{file_name}", cls="hover:underline font-medium"),
            Span(date, cls="text-muted-foreground text-sm whitespace-nowrap"),
            cls="flex justify-between items-baseline gap-4 py-2 border-b"
        )
    
    return Section(
        Div(
            H3("Latest Posts", cls="text-2xl font-semibold"),
            A("View all →", href="/blogs", cls="text-primary text-sm hover:underline"),
            cls="flex justify-between items-baseline mb-4"
        ),
        *[post_item(m) for m in sorted_posts],
        cls="border rounded-lg shadow-sm bg-gray-50 p-4"
    )
...
def create_blog_card_from_metadata(meta_dict, blog_id):
    ...
    return Div(
        ...
        meta_row,
        cls="space-y-2 border-b pb-4 mb-4 group p-4 -mx-4 hover:rounded-lg hover:shadow-md hover:border-transparent transition-all cursor-pointer"
    )
...
def display_post(filename):
    ...
    # Using inline styles to guarantee they apply (CSS was being overridden)
    return Div(
        Div(create_navbar(), cls="w-full max-w-2xl mx-auto px-4"),
        Div(
            # Left column: Main content (constrained width for readability)
            Article(
                H1(meta_dict.get('title', 'Untitled Post')),
                Div(
                    Span(meta_dict.get('date', ''), cls="text-sm text-muted-foreground"),
                    tag_pills,
            ...
            Aside(toc, style="position: sticky; top: 2rem; font-size: 13px; line-height: 1.5; color: #6b7280; max-height: calc(100vh - 4rem); overflow-y: auto;"),
            style="display: grid; grid-template-columns: minmax(0, 640px) 200px; gap: 5rem; max-width: 1050px; margin: 0 auto; padding: 1.5rem 2.5rem; align-items: start;"
        ),
        create_footer(),
